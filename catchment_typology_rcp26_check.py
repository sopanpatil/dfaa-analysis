# catchment_typology_rcp26_check.py
# ------------------------------------
# RCP2.6 robustness check for the FTD catchment typology (Supplementary
# Information Section S2). Re-derives the same k-means/Ward clustering
# as catchment_typology.py, but on RCP2.6 delta metrics (the lowest-
# emissions, weakest-signal scenario) instead of RCP8.5, to test whether
# the RCP8.5-defined partition reflects a stable catchment property or
# is specific to the RCP8.5 signal. Writes to a separate output
# directory (typology_output_rcp26_check/) so the original RCP8.5
# typology outputs (typology_output/) are never overwritten.
#
# Clustering variables (RCP2.6 delta metrics, z-score standardised):
...
#
# Outputs (in typology_output_rcp26_check/)
# ------------------------------------------
...
# Usage
# -----
#   python catchment_typology_rcp26_check.py
#   python catchment_typology_rcp26_check.py --n-clusters 3   # force k=3
#
# Method note
# -----------
# K-means is the primary method (reported in main paper for RCP8.5).
...
ATTRS_DIR      = Path(".")
OUTPUT_DIR     = Path("./typology_output_rcp26_check")  # SEPARATE from original RCP8.5 output

RCPS      = ["rcp26", "rcp45", "rcp60", "rcp85"]
CLUSTER_RCP = "rcp26"   # ROBUSTNESS CHECK: lowest-emissions scenario, weakest signal
...
def main():
    ...
    # Summary printout
    # ------------------------------------------------------------------
    print(f"\n--- Cluster profiles ({CLUSTER_RCP.upper()} delta metrics) ---")
    cluster_rcp_profiles = profiles[profiles["rcp"] == CLUSTER_RCP]
    for var in CLUSTER_VARS:
        sub = cluster_rcp_profiles[cluster_rcp_profiles["variable"] == var]
