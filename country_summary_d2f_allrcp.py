...
#
# This compares countries directly on the continuous delta metric values
# (Kruskal-Wallis / Mann-Whitney), independent of and complementary to
# the cluster-based typology comparison in catchment_typology_d2f.py /
# Section 3.2:
...
#      England vs Wales) for delta_gap specifically, since that's the
#      metric with the clearest visual country pattern.
#
# Requires:
...
# Usage
# -----
#   python country_summary_d2f_allrcp.py --rcp rcp85
...
def parse_args():
    p = argparse.ArgumentParser(description="Per-country summary of d2f delta metrics.")
...
def run_one_rcp(rcp, boundary):
    ...
    print("\n" + "=" * 90)
    print(f"PER-COUNTRY SUMMARY -- d2f delta metrics, {rcp}")
    print("=" * 90)
    summary_df = pd.concat(all_summaries, ignore_index=True)[
        ["metric", "country", "n", "median", "q25", "q75"]
    ]
    print(summary_df.to_string(index=False))
    summary_df.to_csv(OUTPUT_DIR / f"d2f_country_summary_{rcp}.csv", index=False)
    ...
    kw_df.to_csv(OUTPUT_DIR / f"d2f_kruskal_wallis_{rcp}.csv", index=False)

    for metric in ["delta_gap", "delta_drought_dur", "delta_flood_peak", "delta_recovery_rate"]:
        if metric not in df.columns:
            continue
        print("\n" + "=" * 90)
        ...
        pairwise.to_csv(OUTPUT_DIR / f"d2f_{metric}_pairwise_{rcp}.csv", index=False)
